...
// กำหนด Base URL ของ API จาก .env
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL;
const GO_BASE_URL = API_BASE_URL.replace('/api/redirects', '/go/');
...
function RedirectList() {
  ...
  const [editingId, setEditingId] = useState(null); // ID ของ Company ที่กำลังแก้ไข
  const [currentEditData, setCurrentEditData] = useState(null); // ข้อมูลของ Company ที่กำลังแก้ไข
  ...
  const fetchRedirects = async () => {
    ...
    try {
      ...
      setError('Failed to fetch redirects. Please check server connection.');
  ...
  const handleEditCompany = (redirect) => {
    ...
    // สร้าง deep copy เพื่อป้องกันการแก้ไข state โดยตรง
  ...
  const handleSaveCompany = async () => {
    ...
    try {
      ...
      // Update the main redirects state with the saved data
